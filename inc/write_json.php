...
// an array to store the new book to be added to the file
$newBook = [
...
$books = json_decode(file_get_contents($file));     // books is now an opject of the file's JSON representation

// only execute if our variable is the object we expect
if (is_object($books->collection->books[0])) {
    // add the new book to our book collection
    $books->collection->books[] = $newBook;
}
